...
class CurvatureBlock(utils.Finalizable):
  ...
  def parameters_canonical_order(self) -> tuple[int, ...]:
    ...
    if meta.params_canonical_order is not None:
      return meta.params_canonical_order

    # Before JAX 0.11, Var.count encoded the global Jaxpr variable order.
    # Retain this fallback for blocks constructed outside ProcessedJaxpr on
    # older JAX versions. ProcessedJaxpr records the semantic parameter order
    # above, because JAX 0.11 removed Var.count entirely.
    counts = [getattr(p, "count", None) for p in self.parameter_variables]
    if all(count is not None for count in counts):
      return tuple(np.argsort(counts))
    raise ValueError(
        "Canonical parameter order is unavailable. Construct curvature blocks "
        "from a ProcessedJaxpr so global parameter indices are recorded."
    )
