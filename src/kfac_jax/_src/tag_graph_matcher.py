...
_JAXPR_TYPES_MERGED = Jaxpr is ClosedJaxpr
...
def _closed_jaxpr(jaxpr: Jaxpr, consts: Sequence[Any]) -> ClosedJaxpr:
  """Attaches constant values using the cross-version constructor form.

  JAX 0.11 merged ``Jaxpr`` and ``ClosedJaxpr`` and removed support for
  ``ClosedJaxpr(jaxpr=..., consts=...)`` keyword construction.  The legacy
  positional form remains supported in both JAX 0.10 and 0.11.
  """
  return ClosedJaxpr(jaxpr, consts)


def _scan_num_consts(eqn: JaxprEqn) -> int:
  """Returns the number of scan constants across JAX 0.10 and 0.11."""
  if "num_consts" in eqn.params:
    return eqn.params["num_consts"]
  # JAX 0.11 represents the (const, carry, xs) input partition as a FlatTree.
  ft_in = eqn.params.get("ft_in")
  if ft_in is None:
    raise KeyError("scan equation has neither num_consts nor ft_in")
  consts, _, _ = ft_in.unpack()
  return len(consts)
...
def to_closed_jaxpr(jaxpr: JaxprOrClosedJaxpr) -> ClosedJaxpr:
  if _JAXPR_TYPES_MERGED:
    # In JAX 0.11 every Jaxpr carries its own (possibly empty) const values.
    # Re-wrapping it would both be redundant and risk discarding those values.
    return jaxpr
  if isinstance(jaxpr, Jaxpr):
    return _closed_jaxpr(jaxpr, [])
  return jaxpr
    ...
    for eqn, n in reversed(self.parent_equations):
      ...
      if eqn.primitive.name == "cond":
        ...
        p_indexes = [eqn.params["jaxpr"].jaxpr.invars.index(p)
                     for p in param_vars]
        checks = [pi < _scan_num_consts(eqn) for pi in p_indexes]
