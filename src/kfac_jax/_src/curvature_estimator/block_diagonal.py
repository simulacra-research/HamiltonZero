...
class BlockDiagonalCurvature(
  ...
  def __init__(
      ...
      shared_forward_value_func: utils.Func | None = None,
      **kwargs: Any,
  ):
    ...
    Args:
      ...
      should_vmap_samples: Whether to use ``jax.vmap`` to compute samples
        when ``num_samples > 1``.
      shared_forward_value_func: Optional tagged function returning
        ``(loss, gradient_surrogate)``. The surrogate's parameter gradient
        must be the training gradient associated with ``loss``. When provided,
        exact-Fisher curvature and the training gradient can share one model
        primal evaluation.
    ...
    if shared_forward_value_func is None:
      self._vjp_and_value_and_grad = None
      self._value_and_grad_jaxpr_extractor = None
    else:
      (
          self._vjp_and_value_and_grad,
          self._value_and_grad_jaxpr_extractor,
      ) = tracer.layer_tags_vjp_and_value_and_grad(
          value_func=shared_forward_value_func,
          params_index=self.params_index,
          auto_register_tags=auto_register_tags,
          **self._auto_register_kwargs,
      )
  ...
  def param_order(self):
    block_order = tuple(
        index
        for block_indices in self.jaxpr.layer_indices
        for index in block_indices
    )
    return np.argsort(block_order)
  ...
  def _finalize(self, func_args: utils.FuncArgs):
    ...
    if self._value_and_grad_jaxpr_extractor is not None:
      shared_jaxpr = self._value_and_grad_jaxpr_extractor(func_args)

      def aval_signature(var):
        return (tuple(var.aval.shape), var.aval.dtype)

      def layer_signature(tag):
        meta = tag.params.get("meta")
        data = tag.primitive.layer_data(tag.invars, tag.params)
        return (
            getattr(meta, "variant", None),
            getattr(meta, "inputs_index", None),
            getattr(meta, "outputs_index", None),
            getattr(meta, "params_index", None),
            getattr(meta, "params_canonical_order", None),
            tuple(aval_signature(var) for var in data.inputs),
            tuple(aval_signature(var) for var in data.outputs),
            tuple(aval_signature(var) for var in data.params),
        )

      def loss_signature(tag):
        meta = tag.params.get("meta")
        if not isinstance(meta, tags.LossMetaData):
          return (type(meta),)
        return (
            meta.loss_class,
            meta.parameter_dependants,
            meta.parameter_independants,
            meta.argument_names,
            tuple(aval_signature(var) for var in tag.invars),
        )

      shared_layers = tuple(
          (tag.primitive, layer_signature(tag))
          for tag in shared_jaxpr.layer_tags
      )
      reference_layers = tuple(
          (tag.primitive, layer_signature(tag))
          for tag in self._jaxpr.layer_tags
      )
      if (
          shared_jaxpr.layer_indices != self._jaxpr.layer_indices
          or shared_layers != reference_layers
      ):
        raise ValueError(
            "The shared-forward graph changed layer registrations."
        )
      shared_losses = tuple(
          loss_signature(tag) for tag in shared_jaxpr.loss_tags
      )
      reference_losses = tuple(
          loss_signature(tag) for tag in self._jaxpr.loss_tags
      )
      if shared_losses != reference_losses:
        raise ValueError(
            "The shared-forward graph changed loss registrations."
        )
    self._create_blocks()
  ...
  def _update_exact_curvature_matrix_estimate(
      self,
      *,
      losses,
      losses_vjp,
    ...
    """Updates exact Fisher/GGN factors from an already-built layer VJP."""

    zero_tangents = jax.tree_util.tree_map(
        jnp.zeros_like,
        list(loss.parameter_dependants for loss in losses),
    )
    if estimation_mode == "fisher_exact":
      inner_shapes = [loss.fisher_factor_inner_shape[1:] for loss in losses]
    else:
      inner_shapes = [loss.ggn_factor_inner_shape[1:] for loss in losses]

    total_num_indices = sum(utils.product(shape) for shape in inner_shapes)
    ...
    for i, (loss, shape) in enumerate(zip(losses, inner_shapes)):
      for index in np.ndindex(shape):
        ...
        vjp_vec[i] = jax.tree_util.tree_map(
            lambda x: x * jnp.sqrt(total_num_indices),
            vjp_vec[i],
        )
  ...
  def update_curvature_matrix_estimate_and_value_and_grad(
  ...
  ) -> tuple[State, Array, utils.Params]:
    """Updates exact Fisher factors and returns loss/grads from one forward."""

    del rng, pmap_axis_name
    if not self.finalized:
      self.finalize(func_args)
    ...
    if estimation_mode != "fisher_exact":
      raise ValueError(
          "Shared curvature/value-and-grad execution supports only "
          f"`fisher_exact`; got {estimation_mode!r}."
      )
    if self._vjp_and_value_and_grad is None:
      raise ValueError(
          "This estimator has no `shared_forward_value_func`."
      )

    losses, losses_vjp, loss, grads = self._vjp_and_value_and_grad(func_args)
    if any(
        not isinstance(loss_obj, loss_functions.NegativeLogProbLoss)
        for loss_obj in losses
    ):
      raise ValueError(
          "One of the losses is incompatible with `fisher_exact`."
      )
    state = self._update_exact_curvature_matrix_estimate(
        losses=losses,
  ...
  def update_curvature_matrix_estimate(
    ...
    if estimation_mode == "fisher_gradients":
      ...
      return self._update_exact_curvature_matrix_estimate(
          losses=losses,
          losses_vjp=losses_vjp,
          state=state,
          ema_old=ema_old,
